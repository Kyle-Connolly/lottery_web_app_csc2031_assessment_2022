...
@requires_roles('admin')  # admin access only
def admin():
...
def run_lottery():
    ...
    if current_winning_draw:
        # decrypt winning draw using admin's key
        current_winning_draw.view_numbers(current_user.lottokey)  # ADDED
        ...
        if user_draws:
            ...
            for draw in user_draws:
                ...
                # decrypt winning user draw using user's key
                draw.view_numbers(user.lottokey)  # ADDED
